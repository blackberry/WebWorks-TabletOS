...
/**
 * Class <code>ErrorBuffer</code> uses a separate thread to read from the
 ...
 * @see ProcessBuffer
 */
public final class ErrorBuffer extends ProcessBuffer {

    /**
     * The contents as read from the standard error stream, or
     * <code>null</code> if either the buffer isn't available yet or
     * the error stream could not be completely read.
     ...
     */
    public ErrorBuffer(Process process) {
    ...
    /**
     * Returns the contents as read from the standard error stream, or
     * <code>null</code> if either the buffer isn't available yet or
     * the error stream could not be completely read.
     *
     * @return the contents as read from the standard error stream, or
     * <code>null</code> if either the buffer isn't available yet or
     * the error stream could not be completely read.
     */
    ...
    /**
     * Reads to the end of the error stream and then fills this buffer
     * with the result.
     */
    void fill() {
        try {
            InputStream is = getProcess().getErrorStream();
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            for (int b; (b = is.read()) != -1; baos.write(b))
                ;
            synchronized (getBufferSync()) {
                _stderr = baos.toByteArray();
            }
        } catch (IOException ioe) {
            // do nothing - getStderr() will return null
        }
    }
}
