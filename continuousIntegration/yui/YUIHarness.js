			...
			*/
			runSuite : function(theSuite) {
			    ...
			    function handleTestComplete(data) {
					...
					if (YUI.framework.isContinuousIntegrationRun)
						continuousIntegrationTestingComplete();
					...
					if (document.getElementById("cbReportQC").checked && document.getElementById("tbUserName").value && document.getElementById("tbPassword").value){
						...
						document.getElementById("cbReportQC").checked = false;
						$.blockUI({ message: '<h1>Pushing results to QC...</h1><h3>(This may take a while)</h3>' });
						$.ajax({
							type: "POST",
							...
							success: function(data, textStatus, XMLHttpRequest) {
								$.unblockUI();
								if (data.d == "Success")
									$.growlUI("QC Report", "Completed Successfully!"); 
								else
									$.growlUI("QC Report", "Failure: " + data.d); 
							},
							failure: function(errMsg) {
								$.unblockUI();
								$.growlUI("QC Report", "Failure: " + errMsg); 
							}
						});
					}
				}
			...
			displayResultByFormat : function(_format) {
				...
				return formattedTestingResultStr
			},
